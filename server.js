...
function updatePlayer(player, input, dt) {
  ...
  const jumpPressed = Boolean(input.up) && !player.previousUp;
  ...
  if (player.dashTime > 0) {
    ...
    if (jumpPressed && (player.onGround || player.airJumpsRemaining > 0)) {
      player.vy = -WORLD.jumpVelocity;
      if (!player.onGround) player.airJumpsRemaining -= 1;
  ...
  const sideBounds = player.side === 'left'
  ...
  player.x = clamp(player.x, sideBounds[0], sideBounds[1]);
  ...
  player.previousUp = Boolean(input.up);
}
